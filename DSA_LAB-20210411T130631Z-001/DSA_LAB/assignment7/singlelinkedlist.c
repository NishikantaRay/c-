...
void in_before(int key)
{
	...
	for(temp=start;temp!=NULL&&(temp->add)->info!=key;temp=temp->add);
	if(temp==NULL)
	{
		printf("\nELEMENT NOT FOUND\n");
		return;
...
void sort()
{
	...
	{
		min=temp->info;
		for(t1=temp->add;t1!=NULL;t1=t1->add)
			if(min>t1->info)
			{
				t=temp->info;
	...
	while(1)
	{
		...
		switch(ch)
		{
			...
			case 14: printf("Enter the position: ");
				...
				display();
				break;
			case 15: printf("Enter the specific position: ");
				scanf("%d",&key);
				printf("\nDELETED ELEMENT IS: %d\n",del_speci(key));
				display();
				break;
